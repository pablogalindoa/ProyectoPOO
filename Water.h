...
private:
    void reloadData(vector<Vertex>& vertices) {
		...
		static float textureSpeed = -0.5f; // Speed of texture movement
		static float textureOffset = 0.0f;
		textureOffset += textureSpeed;
...
//          vertex.TexCoords = glm::vec2((float)x / (numSegmentsX - 1), (float)z / (numSegmentsZ - 1));
			vertex.TexCoords = glm::vec2(vertex.TexCoords.x + textureOffset, vertex.TexCoords.y);
        }
